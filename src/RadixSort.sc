...
object RadixSort {
  ...
  /**
    ...
    * As it matches the patters it is increasing the count value. This value is returned
    * and then each number is divided by 10 to the power of the counter. Each value corresponds
    * to another significant figure. If the value is less, then it is disregarded. If the resulting value is more
    * then it is repeated with a higher exponent.
